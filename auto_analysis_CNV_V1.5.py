...
## 2019/04/16 version V1.2
## make_sample_json updata to get_project_info_list_for_CNV_V1.4.4.py

## 2019/04/18
## added _PGS_ to auto_monitor and auto_analysis

## 2019/04/28/
## try except to deal when PE dem is finished,the programme exit

## 2019/05/05
## when no do SE dem,PE also to do auto analysis
...
def judge_new_dempath(dem_path_file):
    ...
    auto_analysis_type = ['_CNV_','_PGS_']
            ...
            if os.path.exists(dem_01):
                if dem_01 not in new_dem_do_anlysis_path:
                    try:
                        ...
                        limscsv = glob.glob("/data/lims_test/SZ/ILLUMINA/" + lims_paltfrom + '/' + '*' + templist[-1] + '*/*_[0-9][0-9].csv')[0]
                        lims_dir = '/'.join(limscsv.split('/')[:-1])

                        for project_dir in os.listdir(lims_dir):
                            for type_01 in auto_analysis_type:
                                if re.search(type_01,project_dir):
                                    if dem_01.endswith('_R1') or  re.search('_SE',lims_dir.split('/')[-1]):
                                    ...
                                    elif not dem_01.endswith('_R1') and re.search('_PE',lims_dir.split('/')[-1]):
                                        dem_01_R1 = dem_01 + "_R1"
                                        if dem_01_R1 in new_dem_do_anlysis_path.keys():
                                            new_dem_do_anlysis_path[dem_01]['dir'] = lims_dir
                                            new_dem_do_anlysis_path[dem_01]['platform'] = lims_paltfrom
                                            new_dem_do_anlysis_path[dem_01]['csv'] = limscsv
                                        else:
...
def prepare_do_anlysis(new_dem_do_anlysis_path,email):
    ...
    for dem_02 in new_dem_do_anlysis_path.keys():
        ...
        run_id_CNV_dir = "/data/Project/Project_run_log/" + platform_index[new_dem_do_anlysis_path[dem_02]['platform']] + '_' +  run_id + "_PGS_and_CNV"
        ...
        try:
            ...
            os.system("cd {dir} && python3 /data/software/bioinfo_practical_applications/auto_analysis_CNV/auto_PGS_CNV/get_project_info_list_for_CNV_PGS_V1.4.4.py --samplesheet_path {csv}".format(dir=run_id_CNV_dir,csv=str(new_dem_do_anlysis_path[dem_02]['csv'])))
            ...
            os.system("/data/software/bin/perl /data/software/Pipeline/auto_submit_pipeline/bin1/write_chromgo_shell_v5.1.1.pl {analysis_dir} {run_dir} {run_NO} {local_lims_dir} {Project_info_list} {emailreport}".format(
            analysis_dir="/data/Project/",run_dir=run_id_CNV_dir,run_NO=run_id,local_lims_dir=new_dem_do_anlysis_path[dem_02]['dir'],Project_info_list=run_id_CNV_project_list,emailreport=email))
